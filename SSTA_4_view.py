...
def run_steps(x_batch, model_0, model_1, vae, inference=True, args=None, model_2=None, model_3=None):
    ...
    if args.eval_mode == 'multi_step_eval'and inference == True:  # and inference == True
        with torch.no_grad():
            ...
            for t in range(args.eval_num_step + args.num_past):
                x_0_t_pred, message_0, memory_0, h_t_0, c_t_0, delta_c_list_0, delta_m_list_0 = \
                    model_0(x_0_t_pred_prev, message_0, message_1, memory_0, h_t_0, c_t_0, delta_c_list_0,
                            delta_m_list_0, [message_2, message_3])
                if args.message_type in ['vae']:
                    ...
                    if t < args.num_past or use_gt_flag: # or t % args.mask_per_step == 0:
                        message_0 = vae.get_message(x_0_t[:, t:t + 1]).detach()
                    else:
                        message_0 = vae.get_message(x_0_t_pred_prev.detach()).detach()
                    ...
                    message_0 = torch.zeros((x_batch.shape[0], 1, x_batch.shape[2], x_batch.shape[3], 1)).to(args.device)  # torch.zeros_like(message_0)
                ...
                x_1_t_pred, message_1, memory_1, h_t_1, c_t_1, delta_c_list_1, delta_m_list_1 = \
                    model_1(x_1_t_pred_prev, message_1, message_0, memory_1, h_t_1, c_t_1, delta_c_list_1,
                            delta_m_list_1, [message_2, message_3])
                if args.message_type in ['vae']:
                    if t < args.num_past or use_gt_flag: # or t % args.mask_per_step == 0:
                        message_1 = vae.get_message(x_1_t[:, t:t + 1]).detach()
                    else:
                        message_1 = vae.get_message(x_1_t_pred_prev.detach()).detach()
                elif args.message_type in ['raw_data']:
                    if t < args.num_past or use_gt_flag: # or t % args.mask_per_step == 0:
                    ...
                    message_1 = torch.zeros((x_batch.shape[0], 1, x_batch.shape[2], x_batch.shape[3], 1)).to(args.device)  # torch.zeros_like(message_1)
                elif args.message_type == 'randn':
                ...
                if args.num_views in [4, ]:
                    x_2_t_pred, message_2, memory_2, h_t_2, c_t_2, delta_c_list_2, delta_m_list_2 = \
                        model_2(x_2_t_pred_prev, message_1, message_0, memory_2, h_t_2, c_t_2, delta_c_list_2,
                                delta_m_list_2, [message_2, message_3])
                    if args.message_type in ['vae']:
                        if t < args.num_past or use_gt_flag:  # or t % args.mask_per_step == 0:
                            message_2 = vae.get_message(x_2_t[:, t:t + 1]).detach()
                        else:
                            message_2 = vae.get_message(x_2_t_pred_prev.detach()).detach()
                    elif args.message_type in ['raw_data']:
                        if t < args.num_past or use_gt_flag:  # or t % args.mask_per_step == 0:
                        ...
                        message_2 = torch.zeros((x_batch.shape[0], 1, x_batch.shape[2], x_batch.shape[3], 1)).to(args.device)  # torch.zeros_like(message_2)
                    elif args.message_type == 'randn':
                    ...
                    x_3_t_pred, message_3, memory_3, h_t_3, c_t_3, delta_c_list_3, delta_m_list_3 = \
                        model_3(x_3_t_pred_prev, message_1, message_0, memory_3, h_t_3, c_t_3, delta_c_list_3,
                                delta_m_list_3, [message_2, message_3])
                    if args.message_type in ['vae']:
                        if t < args.num_past or use_gt_flag:  # or t % args.mask_per_step == 0:
                            message_3 = vae.get_message(x_3_t[:, t:t + 1]).detach()
                        else:
                            message_3 = vae.get_message(x_3_t_pred_prev.detach()).detach()
                    elif args.message_type in ['raw_data']:
                        if t < args.num_past or use_gt_flag:  # or t % args.mask_per_step == 0:
                        ...
                        message_3 = torch.zeros((x_batch.shape[0], 1, x_batch.shape[2], x_batch.shape[3], 1)).to(args.device)  # torch.zeros_like(message_3)
                    elif args.message_type == 'randn':
                ...
                if t < args.num_past: # or t % args.mask_per_step == 0:
                    ...
                    x_0_t_pred_prev = x_0_t[:, t + 1:t + 2]  # torch.cat([x_0_t_pred_prev[:, 1:], x_0_t_pred], 1)
                    x_1_t_pred_prev = x_1_t[:, t + 1:t + 2]
                    pred_batch_0_list.append(x_0_t_pred.detach().cpu())
                    pred_batch_1_list.append(x_1_t_pred.detach().cpu())
                    message_0_list.append(message_0.detach().cpu())
                    message_1_list.append(message_1.detach().cpu())
                    if args.num_views in [4, ]:
                        x_2_t_pred_prev = x_2_t[:, t + 1:t + 2]  # torch.cat([x_0_t_pred_prev[:, 1:], x_0_t_pred], 1)
                        x_3_t_pred_prev = x_3_t[:, t + 1:t + 2]
                        pred_batch_2_list.append(x_2_t_pred.detach().cpu())
                        pred_batch_3_list.append(x_3_t_pred.detach().cpu())
                        message_2_list.append(message_2.detach().cpu())
                        message_3_list.append(message_3.detach().cpu())
                else:
            ...
            pred_batch_0 = torch.cat(pred_batch_0_list, 1).cpu()
            pred_batch_1 = torch.cat(pred_batch_1_list, 1).cpu()
            if args.num_views in [4, ]:
                pred_batch_2 = torch.cat(pred_batch_2_list, 1).cpu()
                pred_batch_3 = torch.cat(pred_batch_3_list, 1).cpu()
            ...
            message_batch_0 = torch.cat(message_0_list, 1).cpu()
            message_batch_1 = torch.cat(message_1_list, 1).cpu()
            if args.num_views in [4, ]:
                message_batch_2 = torch.cat(message_2_list, 1).cpu()
                message_batch_3 = torch.cat(message_3_list, 1).cpu()
        ...
        if args.mode == 'train':
            ...
            while (train_input_handle.no_batch_left() == False):
                ...
                pred_batch, message_batch = run_steps(x_batch, model_0, model_1, vae,
                                                      ...
                                                      model_2=model_2, model_3=model_3)
        ...
        if False and args.mode == 'eval':
            ...
            with torch.no_grad():
                ...
                while (test_input_handle.no_batch_left() == False):
                    ...
                    y_test_torch, message_torch = run_steps(x_test_torch, model_0, model_1, vae,
                                                            inference=True, args=args,
                                                            model_2=model_2, model_3=model_3)
        if epoch % 100 == 1:
            print("Ep/MaxEp     tr_loss     te_loss")

        if epoch % 10 == 0 and args.mode == 'eval':  # 20
            batch_id = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_num_step', type=int, default=300, help='10, ')
    parser.add_argument('--eval_per_step', type=int, default=3)
    ...
    parser.add_argument('--data_name', type=str, default="carla_town02_8_view_20220713_color_split_4",
    ...
    parser.add_argument('--ckpt_dir', type=str, default='results/20220714-040203/carla_town02_8_view_20220713_color_split_4/', help='checkpoint dir: dir/model_1.pt')
