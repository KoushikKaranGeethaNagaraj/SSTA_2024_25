...
def training(N, Nte, bs, n_epoch, act, data_mode, args):
    ...
    optimizer = optim.Adam(list(model_0.parameters()),lr=0.0001)
    MSE = nn.MSELoss()
    tr_loss = []
    te_loss = []
    ...
    print("savefilepath",root_res_path)
...
#         tr_kl_loss.append(ave_kl_loss.cpu())
        ...
        if epoch % 2==0: # 20
            batch_id = 0
            res_path = os.path.join(root_res_path, "evaluation_images")
        ...
        print("best_loss",best_eval_loss,"avg_te",avg_te_sum_loss)

        if avg_te_sum_loss > best_eval_loss and epoch>1 and args.mode=="train":
            print('avg_te_sum_loss: {}, best_eval_loss: {}'.format(avg_te_sum_loss, best_eval_loss))
            best_eval_loss = avg_te_sum_loss
            print(root_res_path,"XXXXXXXXXXXXXXXXSAVINGMODELVAEXXXXXXXXXXXXXXXXXXXXXXXXXX")
            train_root_res_path=os.path.join(root_res_path, "best_model_vae")
        ...
        if epoch%2==0 and  args.mode=="train":#10
            print("XXXXXXXXXXXXXXXXVAE TRAIN Model XXXXXXXXXXXXXXXXXXXXXXXXXX")
            backup_train_path=os.path.join(root_res_path, "backup_train_vae")
    ...
    print("latest_backup_save")
    torch.save(model_0, os.path.join(root_res_path, "backup_vae.pt"))
    print("END")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpt_dir', type=str,default="/storage/home/hcoda1/9/knagaraj31/p-skousik3-0/SSTA_2022/sim_vae_files/vae_sim_model_32_16_8/backup_train_vae/30", help='checkpoint dir: dir/vae.pt')
